...
class FluentLicensing
{
    ...
    public function getLicenseMessages()
    {
        ...
        if ($status === 'disabled') {
            return [
                'message'         => sprintf(__('The license for %s has been disabled. Please contact support for assistance.', 'fluent-support-pro'), $this->getConfig('plugin_title')),
                'type'            => 'global',
                'license_details' => $licenseDetails
            ];
        }
        ...
        if ($status != 'valid') {
            return [
                'message'         => \sprintf(
                    'The %1$s license needs to be activated.',
    ...
    private function getExpireMessage($licenseData, $scope = 'global')
    {
        ...
        return '<p>Your ' . $this->getConfig('plugin_title') . ' ' . __('license has', 'fluent-support-pro') . ' <b>' . __('expired at', 'fluent-support-pro') . ' ' . gmdate('d M Y', strtotime($licenseData['expires'])) . '</b>, ' . __('Please', 'fluent-support-pro') . ' ' .
            '<a href="' . $renewUrl . '"><b>' . __('Click Here to Renew Your License', 'fluent-support-pro') . '</b></a>' . '</p>';
    }
