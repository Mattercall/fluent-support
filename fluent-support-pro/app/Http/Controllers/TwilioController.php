...
class TwilioController extends Controller
{
    ...
    public function handleResponse(Request $request, $token)
    {
        ...
        $dbToken = Meta::where('object_type', 'integration_settings')
            ->where('key', '_twilio_webhook_token')
            ->first();

        if (!$dbToken || $dbToken->value != $token) {
            return $this->sendError([
                'message' => __('Invalid webhook token', 'fluent-support-pro')
            ]);
        }
