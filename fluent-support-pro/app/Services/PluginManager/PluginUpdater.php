    ...
    private function initPluginUpdaterHooks()
    {
        ...
        if ($this->config['show_check_update']) {
            ...
            add_filter('plugin_row_meta', function ($links, $file) use ($getParam) {
                ...
                $row_meta = array(
                    'check_update' => '<a  style="color: #583fad;font-weight: 600;" href="' . $checkUpdateUrl . '" aria-label="' . esc_attr__('Check Update', 'fluent-support-pro') . '">' . __('Check Update', 'fluent-support-pro') . '</a>',
                );
