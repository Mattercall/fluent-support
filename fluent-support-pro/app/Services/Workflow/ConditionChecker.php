...
class ConditionChecker
{
    public function check($workFlow, $source, $customer)
    {
        ...
        $isTriggeredByTicket = strpos($workFlow->trigger_key, 'ticket') !== false;
    ...
    private function match($condition, $target)
    {
        ...
        switch ($condition['data_operator']) {
            case 'contains':
                ...
                return mb_stripos($target->{$condition['property']}, (string) $condition['data_value']) !== false;
            case 'not_contains':
                if (!$this->isPropertyExist($target, $condition['property'])) {
                    return false;
                }
                return mb_stripos($target->{$condition['property']}, (string) $condition['data_value']) === false;
            case 'yes':
                return $target->attachments && $target->attachments->count();
            case 'no':
                return !$target->attachments || !$target->attachments->count();
            case 'range':
    ...
    private function matchCRMTagsOrLists($type, $condition, $customer)
    {
        ...
        if ($type == 'lists') {
            return $crmUser->hasAnyListId($condition);
        } else {
            return $crmUser->hasAnyTagId($condition);
        }
    }
    ...
    private function isPropertyExist($object, $target)
    {
        ...
        if ($object instanceof \FluentSupport\App\Models\Model) {
