...
class Updater
{
    ...
    function plugins_api_filter($_data, $_action = '', $_args = null)
    {
        ...
        if(!in_array($_args->slug, [$this->slug, 'fluent-support-pro'])) {
            return $_data;
        }
    ...
    private function api_request($_action, $_data)
    {
        ...
        if ($request && isset($request->sections)) {
            $request->sections = Helper::safeUnserialize($request->sections);
    ...
    private function maybe_delete_transients()
    {
        ...
        if(isset($_GET['fluentsupport_pro_check_update'])) {
            if ( current_user_can( 'update_plugins' ) ) {
                $this->delete_transients();
                ...
                wp_redirect(admin_url('plugins.php?s=fluent-support-pro&plugin_status=all'));
                exit();
            }
        }
